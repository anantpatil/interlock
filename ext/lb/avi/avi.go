...
func NewDockerTask(serviceName string, portType string, ipAddr string, publicPort uint16, privatePort uint16) *dockerTask {
	return &dockerTask{serviceName, portType, ipAddr, int(publicPort), int(privatePort)}
}
