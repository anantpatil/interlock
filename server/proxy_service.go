...
func (s *Server) getProxyServiceConfig(serviceCluster, version string) (*swarm.Config, error) {
	cfgs, err := s.getProxyServiceConfigs()
	if err != nil {
		return nil, err
	}
...
func (s *Server) createProxyServiceConfig(serviceCluster string, data []byte) (swarm.Config, string, error) {
	cfg := swarm.Config{}
...
func (s *Server) configureProxyService(serviceCluster, version string) error {
	client, err := getDockerClient(s.cfg)
	if err != nil {
		return err
	}
	...
	plugin, ok := s.plugins[serviceCluster]
	if !ok {
		return ErrServiceClusterConfigDoesNotExist
	}

	proxyConfig, err := s.getProxyServiceConfig(serviceCluster, version)
	if err != nil {
		return err
	}
	if proxyConfig == nil {
		return ErrProxyServiceConfigDoesNotExist
	}

	taskSpec := swarm.TaskSpec{
		ContainerSpec: &swarm.ContainerSpec{
			Image: plugin.ProxyImage,
			Configs: []*swarm.ConfigReference{
				{
					File: &swarm.ConfigReferenceFileTarget{
						Name: plugin.ProxyConfigPath,
						UID:  "0",
						GID:  "0",
						Mode: 0644,
					},
					ConfigID:   proxyConfig.ID,
					ConfigName: proxyConfig.Spec.Name,
				},
			},
		},
	...
	if len(plugin.ProxyArgs) > 0 {
		taskSpec.ContainerSpec.Args = plugin.ProxyArgs
	}
	...
	svc, err := s.getProxyService(serviceCluster)
	if err != nil {
		return err
	}
	...
	if svc == nil {
		...
		if err != nil {
			return errors.Wrapf(err, "error creating proxy service: %+v", spec)
		}
		...
		updatedService, err := s.getProxyService(serviceCluster)
		if err != nil {
			return err
		}
...
func (s *Server) getProxyServiceConfigs() ([]swarm.Config, error) {
	cfgs := []swarm.Config{}
	client, err := getDockerClient(s.cfg)
	if err != nil {
		return cfgs, err
